...
enum custom_keycodes {
  ...
  KC_CTRLALT,  // Control-Alt together, for when we need to send both modifiers at once. Useful for
               // Mac desktop selection and more.
...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
	[_QWERTY] = LAYOUT_65_xt_ansi_blocker_tsangan(
		KC_ESC,      KC_CTRLALT, KC_GRAVE,KC_1,    KC_2,    KC_3,    KC_4,    KC_5,    KC_6,    KC_7,    KC_8,    KC_9,    KC_0,    KC_MINS, KC_EQL,  KC_BSPC,          KC_HOME,
...
const char *SCRNSHT2_VALUES[NUM_OS_MODES] = {SS_LGUI(SS_LSFT("s")), SS_LCTL(SS_LCMD(SS_LSFT("4")))};
...
bool process_os_key(uint16_t keycode, keyrecord_t *record) {
  ...
  switch (keycode) {
    ...
    case KC_SCRNSHT:
        send_string(IS_SHIFTED ? SCRNSHT2_VALUES[os_mode] : SCRNSHT_VALUES[os_mode]);
        return STOP;
  }
